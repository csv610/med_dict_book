...
\medterm{Arthrography}-- Contrast injected into joint for imaging. See arthrography.
...
\medterm{Artifacts}-- Image distortions not representing true anatomy. See artifacts.
...
\medterm{Bladder Cancer}-- Urothelial carcinoma. See urology.
...
\medterm{Consolidation}-- Lung parenchyma filled with fluid, detected on imaging. See consolidation.

\medterm{Continuing Medical Education}-- Mandatory CME for board certification maintenance. See continuing education.
...
\medterm{Elastography}-- Imaging tissue stiffness. See elastography.
...
\medterm{Evidence-Based Medicine}-- Practice informed by best available evidence. See evidence-based medicine.
...
\medterm{Hydronephrosis}-- Dilation of renal collecting system proximal to obstruction. See hydronephrosis.
...
\medterm{Hysterosalpingography}-- Contrast imaging of uterus and fallopian tubes. See hysterosalpingography.
...
\medterm{Interventional Radiology}-- Image-guided minimally invasive procedures. See interventional radiology.
...
\medterm{Lymphangiography}-- Contrast imaging of lymphatic vessels. See lymphangiography.
...
\medterm{Patient Communication}-- Exchange of information between provider and patient. See communication.
...
\medterm{PET/CT}-- Combined PET and CT imaging. See nuclear medicine.

\medterm{PET/MRI}-- Combined PET and MRI. See nuclear medicine.
...
\medterm{Quality Control}-- Systematic efforts to enhance care. See quality improvement.

\medterm{Radiation Biology}-- Biological effects of radiation. Deterministic vs. stochastic effects.

\medterm{Radiation Dose}-- Principles: time, distance, shielding. See radiation safety.
...
\medterm{Radiopharmaceutical}-- Radioactive compound used for diagnostic or therapeutic purposes. See nuclear medicine.
...
\medterm{Sialography}-- Contrast imaging of salivary ducts. See sialography.
...
\medterm{Stress Echocardiography}-- Echocardiography during exercise or pharmacological stress. See stress echocardiography.
...
\medterm{Summary}-- Summary of key findings and recommendations in radiology report. See impression.
...
\medterm{Technetium-99m}-- Most widely used radionuclide in nuclear medicine. See nuclear medicine.
...
\medterm{Urolithiasis}-- Stone formation in the urinary tract (kidneys, ureters, bladder, urethra). See urology.
...
\medterm{Xanthogranulomatous Pyelonephritis}-- Chronic destructive renal infection with replacement of parenchyma by lipid-laden macrophages. See urology.
