...
\medterm{Abdominal Aortic Aneurysm Screening}-- Abnormal focal dilation of blood vessel. See aneurysm.

\medterm{Activities of Daily Living}-- Ability to perform activities of daily living (ADLs) and instrumental ADLs (IADLs). See functional status.
...
\medterm{Advance Care Planning}-- Discussion of future healthcare preferences. See advance care planning.
...
\medterm{Anorexia}-- Oral role in nutrition. See nutrition.
...
\medterm{Balance Impairment}-- The ability to maintain the body's center of mass over its base of support, relying on input from the vestibular system, vision, and proprioception. See balance.
...
\medterm{Behavior Change}-- Modification of health-related behaviors. See behavior change.
...
\medterm{Bone Density Screening}-- Systemic skeletal disease characterized by low bone mass and microarchitectural deterioration. See osteoporosis.

\medterm{Bone Health}-- Systemic skeletal disease characterized by low bone mass and microarchitectural deterioration. See osteoporosis.

\medterm{BPH}-- Abbreviation for Benign Prostatic Hyperplasia. See BPH.
...
\medterm{Capacity}-- Autonomous voluntary authorization given by a competent patient to undergo a specific medical intervention, procedure, or clinical trial. See informed consent.
...
\medterm{Cognitive Decline}-- Decline in one or more cognitive domains: memory, attention, language, visuospatial, executive function. See cognitive impairment.
...
\medterm{Cognitive Screening}-- Decline in one or more cognitive domains: memory, attention, language, visuospatial, executive function. See cognitive impairment.
...
\medterm{Competence}-- Autonomous voluntary authorization given by a competent patient to undergo a specific medical intervention, procedure, or clinical trial. See informed consent.
...
\medterm{Decubitus Ulcer}-- Localized damage to the skin and/or underlying soft tissue caused by prolonged pressure, shear, or friction, most commonly over bony prominences. See pressure injury.
...
\medterm{Depression Screening}-- Major depressive disorder is a common, disabling mood disorder characterized by persistent sadness and loss of interest or pleasure in activities, affecting approximately 7 percent of adults worldwide annually. See depression.

\medterm{Disability}-- Ability to perform activities of daily living (ADLs) and instrumental ADLs (IADLs). See functional status.
...
\medterm{Drug Interactions}-- Use of multiple medications (usually 5+). See polypharmacy.
...
\medterm{Dysphagia}-- Difficulty swallowing, classified as oropharyngeal (difficulty initiating a swallow, choking, coughing, nasal regurgitation) or esophageal (sensation of food sticking retrosternally). See dysphagia.
...
\medterm{Elder Abuse}-- Intentional or neglectful act causing harm to older person. See elder abuse.
...
\medterm{Ethics}-- Moral principles guiding practice. See ethics.
...
\medterm{Fracture Prevention}-- Systemic skeletal disease characterized by low bone mass and microarchitectural deterioration. See osteoporosis.
...
\medterm{Frailty}-- Clinical syndrome of decreased reserve and resilience to stressors. See frailty.
...
\medterm{Functional Status}-- Ability to perform activities of daily living (ADLs) and instrumental ADLs (IADLs). See functional status.

\medterm{Funeral}-- Grief after loss. See bereavement.

\medterm{Gait Disturbance}-- Pattern of walking. See gait.
...
\medterm{Health Literacy}-- Ability to obtain, process, understand health information. See health literacy.
...
\medterm{Health-Related QoL}-- Perception of well-being including physical, psychological, social, spiritual domains. See quality of life.
...
\medterm{Hearing Screening}-- Age-related hearing loss. See presbycusis.
...
\medterm{Hormone Therapy}-- Treatment blocking hormone effects in hormone-sensitive cancers. See endocrinology.
...
\medterm{Hospice}-- Palliative care for terminal illness with prognosis <6 months. See hospice.
...
\medterm{Independence}-- Ability to perform activities of daily living (ADLs) and instrumental ADLs (IADLs). See functional status.
...
\medterm{Infection Prevention}-- Measures to prevent healthcare-associated infections: hand hygiene, personal protective equipment (PPE), isolation precautions (contact, droplet, airborne), sterilization, disinfection. See infection control.
...
\medterm{Instrumental ADL}-- Ability to perform activities of daily living (ADLs) and instrumental ADLs (IADLs). See functional status.

\medterm{Instrumental ADL}-- Ability to perform activities of daily living (ADLs) and instrumental ADLs (IADLs). See functional status.

\medterm{Insurance}-- Ability to obtain needed care. See healthcare access.

\medterm{Interpreter}-- Exchange of information between provider and patient. See communication.
...
\medterm{Language Barrier}-- Exchange of information between provider and patient. See communication.

\medterm{Legacy}-- Discussion of future healthcare preferences. See advance care planning.
...
\medterm{Life Expectancy}-- Average years remaining for person at given age. See life expectancy.
...
\medterm{Long-Term Care Insurance}-- Assistance with ADLs/IADLs for extended period. See long-term care.
...
\medterm{Medicaid}-- Ability to obtain needed care. See healthcare access.

\medterm{Medicare}-- Ability to obtain needed care. See healthcare access.
...
\medterm{Mild Cognitive Impairment}-- Decline in one or more cognitive domains: memory, attention, language, visuospatial, executive function. See cognitive impairment.

\medterm{Morbidity}-- Disease or illness in population. See morbidity.
...
\medterm{Mortality}-- Death from disease. See mortality.

\medterm{Mortality Rate}-- Death from disease. See mortality.

\medterm{Motivation}-- Modification of health-related behaviors. See behavior change.

\medterm{Muscle Weakness}-- Age-related loss of muscle mass and function. See sarcopenia.

\medterm{Neglect}-- Intentional or neglectful act causing harm to older person. See elder abuse.
...
\medterm{Nutrition}-- Oral role in nutrition. See nutrition.
...
\medterm{Nutritional Screening}-- Quick assessment of malnutrition risk. See nutritional screening.
...
\medterm{Osteoarthritis}-- The most common joint disease, characterized by progressive cartilage loss, subchondral bone sclerosis, osteophyte formation, and synovial inflammation, affecting knees, hips, hands (DIP/PIP Heberden/Bouchard nodes), spine. See osteoarthritis.
...
\medterm{Osteopenia}-- Systemic skeletal disease characterized by low bone mass and microarchitectural deterioration. See osteoporosis.
...
\medterm{Osteoporosis}-- Systemic skeletal disease characterized by low bone mass and microarchitectural deterioration. See osteoporosis.
...
\medterm{Palliative Care}-- Specialized medical care for serious illness focusing on symptom relief and quality of life. See palliative care.

\medterm{Parenteral Nutrition}-- Intravenous nutrition bypassing GI tract. See clinical nutrition.

\medterm{Patient Education}-- Teaching patients about health conditions and self-management. See patient education.
...
\medterm{Polypharmacy}-- Use of multiple medications (usually 5+). See polypharmacy.
...
\medterm{Pressure Ulcer}-- Localized damage to the skin and/or underlying soft tissue caused by prolonged pressure, shear, or friction, most commonly over bony prominences. See pressure injury.
...
\medterm{Quality of Life}-- Perception of well-being including physical, psychological, social, spiritual domains. See quality of life.
...
\medterm{Research Ethics}-- Moral principles guiding practice. See ethics.
...
\medterm{Sarcopenia}-- Age-related loss of muscle mass and function. See sarcopenia.

\medterm{Screening}-- Testing asymptomatic individuals for disease. See screening.
...
\medterm{Self-Efficacy}-- Patient belief in ability to manage condition. See self-efficacy.
...
\medterm{Skin Integrity}-- Localized damage to the skin and/or underlying soft tissue caused by prolonged pressure, shear, or friction, most commonly over bony prominences. See pressure injury.
...
\medterm{Swallowing}-- Difficulty swallowing, classified as oropharyngeal (difficulty initiating a swallow, choking, coughing, nasal regurgitation) or esophageal (sensation of food sticking retrosternally). See dysphagia.
...
\medterm{Urinary Retention}-- Inability to empty bladder completely. See urinary retention.
...
\medterm{Vision Screening}-- Reduced visual acuity or field. See vision impairment.
...
\medterm{Well-Being}-- Perception of well-being including physical, psychological, social, spiritual domains. See quality of life.
...
\medterm{Wisdom}-- Cognitive engagement throughout life. See lifelong learning.

\medterm{Wound Care}-- Management of traumatic wounds: irrigation, debridement, closure (sutures, staples, glue), dressing. See wound care.
