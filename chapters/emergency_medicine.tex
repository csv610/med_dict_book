...
\medterm{Black Eye}-- Bruising and swelling around the eye, usually caused by blunt trauma and also called periorbital ecchymosis. Assessment should consider orbital fracture, globe injury, retrobulbar hemorrhage, and associated head injury; vision loss, severe pain, diplopia, proptosis, vomiting, or loss of consciousness require urgent evaluation.

\medterm{Black Widow Spider Bite}-- Envenomation by a Latrodectus spider, causing local pain followed by muscle cramping, abdominal or back pain, sweating, hypertension, nausea, or autonomic symptoms. Treatment is supportive with analgesia and muscle-directed therapy; severe cases may require antivenom and monitored care.

\medterm{Acute Mountain Sickness}-- A mild altitude illness caused by rapid ascent, usually producing headache with nausea, fatigue, dizziness, or sleep disturbance. Prevention and treatment include gradual acclimatization, descent when symptoms worsen, oxygen, and appropriate medical therapy.
...
\medterm{Acute Coronary Syndrome (ACS)}-- See Acute Coronary Syndromes. The term includes unstable angina, NSTEMI, and STEMI and describes acute myocardial ischemia requiring urgent assessment.
...
\medterm{Botulism}-- A rare, potentially fatal neuroparalytic disease caused by botulinum toxin, a potent presynaptic neurotoxin produced by Clostridium botulinum, an anaerobic, spore-forming, gram-positive bacillus. See \hyperlink{Infectious Disease}{infectious disease}.
...
\medterm{Carbon Monoxide Poisoning}-- Toxicity from inhalation of carbon monoxide (CO), a colorless, odorless gas binding hemoglobin ~200-300 times more avidly than oxygen, forming carboxyhemoglobin (COHb) and reducing oxygen delivery. See \hyperlink{Toxicology}{toxicology}.
...
\medterm{Cat Scratch Disease}-- A regional lymphadenitis caused by Bartonella henselae, a gramnegative bacillus transmitted to humans through cat scratches, bites, or licks on broken skin, particularly from kittens (which carry higher bacterial loads). See \hyperlink{Infectious Disease}{infectious disease}.
...
\medterm{Coronary Artery Disease}-- Narrowing or occlusion of coronary arteries, usually due to atherosclerosis, reducing blood flow to the myocardium. See \hyperlink{Cardiology}{cardiology}.
...
\medterm{Craniocerebral Injury}-- Trauma to scalp, skull, or brain. See \hyperlink{Head Injury}{head injury}.

\medterm{Critical Care}-- Specialty managing critically ill patients. See \hyperlink{Intensive Care Medicine}{intensive care medicine}.
...
\medterm{Dengue Fever}-- An acute mosquito-borne viral illness caused by dengue virus (DENV-1, -2, -3, -4), a flavivirus transmitted primarily by Aedes aegypti mosquitoes. See \hyperlink{Infectious Disease}{infectious disease}.
...
\medterm{Encephalitis}-- Inflammation of the brain parenchyma, most commonly caused by viral infections (herpes simplex virus, arboviruses, enteroviruses). See \hyperlink{Infectious Disease}{infectious disease}.
...
\medterm{Ethylene Glycol Poisoning}-- Antifreeze poisoning. See \hyperlink{Toxicology}{toxicology}.
...
\medterm{Hospital-Acquired Pneumonia}-- Pneumonia developing > 48 hours after hospital admission. See \hyperlink{Infectious Disease}{infectious disease}.
...
\medterm{Infection}-- Measures to prevent healthcare-associated infections: hand hygiene, personal protective equipment (PPE), isolation precautions (contact, droplet, airborne), sterilization, disinfection. See \hyperlink{Infection Control}{infection control}.
...
\medterm{Leptospirosis}-- A zoonotic bacterial infection caused by spirochaetes of the genus Leptospira, transmitted through contact with water or soil contaminated with the urine of infected animals (rodents, cattle, dogs). See \hyperlink{Infectious Disease}{infectious disease}.

\medterm{Lyme Disease}-- A tick-borne infection caused by Borrelia burgdorferi (Ixodes scapularis, deer tick), endemic to the northeastern and upper midwestern US. See \hyperlink{Infectious Disease}{infectious disease}.
...
\medterm{Meningitis}-- Inflammation of the meninges most commonly caused by bacterial infection (Neisseria meningitidis, Streptococcus pneumoniae, Haemophilus influenzae, Group B Streptococcus). See \hyperlink{Infectious Disease}{infectious disease}.
...
\medterm{Narrow-Complex Tachycardia}-- See \hyperlink{Cardiology}{cardiology}.
...
\medterm{Opioid Overdose}-- A medical emergency characterized by the classic triad of respiratory depression, miosis (pinpoint pupils), and decreased level of consciousness. See \hyperlink{Toxicology}{toxicology}.

\medterm{Organophosphate Poisoning}-- Excess acetylcholine from organophosphate or carbamate poisoning. See \hyperlink{Cholinergic Crisis}{cholinergic crisis}.

\medterm{Pain}-- Emergency pain control: NSAIDs, opioids, ketamine, lidocaine. See \hyperlink{Pain Management}{pain management}.
...
\medterm{Paracetamol Toxicity}-- See \hyperlink{Toxicology}{toxicology}.

\medterm{Pediatric Fever}-- See \hyperlink{Pediatrics}{pediatrics}.
...
\medterm{Plague}-- A bacterial infection caused by Yersinia pestis, transmitted by fleas from rodents. See \hyperlink{Infectious Disease}{infectious disease}.
...
\medterm{Rabies}-- A fatal viral encephalitis caused by rabies virus, transmitted through the bite of infected animals. See \hyperlink{Infectious Disease}{infectious disease}.
...
\medterm{Salicylate Toxicity}-- See \hyperlink{Toxicology}{toxicology}.
...
\medterm{Serotonin Syndrome}-- A potentially life-threatening condition caused by excessive serotonergic activity in the central and peripheral nervous systems, typically resulting from drug interactions or overdose involving serotonergic medications. See \hyperlink{Pharmacology}{pharmacology}.
...
\medterm{Spontaneous Pneumothorax}-- Life-threatening pneumothorax with mediastinal shift. See \hyperlink{Tension Pneumothorax}{tension pneumothorax}.
...
\medterm{Tetanus}-- An acute neurological disease caused by tetanospasmin toxin from Clostridium tetani, introduced through wounds. See \hyperlink{Infectious Disease}{infectious disease}.
...
\medterm{Toxic Encephalopathy}-- See \hyperlink{Toxicology}{toxicology}.

\medterm{Toxic Shock Syndrome}-- An acute, life-threatening illness caused by bacterial exotoxins. See \hyperlink{Infectious Disease}{infectious disease}.
...
\medterm{Unresponsive Coma}-- See \hyperlink{Coma}{coma}.
...
\medterm{Ventricular Tachycardia}-- A wide-complex tachycardia (QRS >0.12 s) originating from the ventricles, with a rate of 100-250 bpm. See \hyperlink{Cardiology}{cardiology}.

\medterm{Wernicke's Encephalopathy}-- See \hyperlink{Toxicology}{toxicology}.
...
\medterm{Wound Infection}-- See \hyperlink{Infection}{infection}.

\medterm{Xylitol Toxicity}-- See \hyperlink{Toxicology}{toxicology}.

\medterm{Yellow Fever}-- A mosquito-borne viral hemorrhagic fever caused by yellow fever virus (Flavivirus), endemic in tropical Africa and South America. See \hyperlink{Infectious Disease}{infectious disease}.

\medterm{Zika Virus}-- A positive-sense RNA virus belonging to the Flaviviridae family that causes mild febrile illness. See \hyperlink{Infectious Disease}{infectious disease}.

\medterm{ABCDE Assessment}-- A structured emergency assessment of Airway, Breathing, Circulation, Disability, and Exposure. Life-threatening abnormalities are identified and treated immediately before proceeding to the next component, with repeated reassessment after interventions.

\medterm{Acute Ischemic Stroke}-- Sudden neurologic dysfunction caused by focal cerebral, spinal, or retinal ischemia. Emergency evaluation distinguishes hemorrhage from ischemia and determines eligibility for reperfusion therapy while supporting airway, glucose, temperature, and blood pressure management.

\medterm{Activated Charcoal}-- An adsorbent administered in selected oral poisonings to reduce gastrointestinal absorption of a toxin. It is not useful for every substance and is unsafe when the airway is unprotected or gastrointestinal motility is severely impaired.

\medterm{Anaphylactic Shock}-- Distributive shock caused by systemic hypersensitivity mediator release, producing vasodilation, increased vascular permeability, and sometimes bronchospasm. Intramuscular epinephrine is first-line treatment, with airway and circulatory support as required.

\medterm{Cardiac Defibrillation}-- Delivery of an unsynchronized electrical shock for ventricular fibrillation or pulseless ventricular tachycardia. It is combined with high-quality CPR and advanced life support according to the cardiac-arrest rhythm and protocol.

\medterm{Chest Tube Thoracostomy}-- Placement of a tube into the pleural space to evacuate air, blood, pus, or other fluid and allow lung re-expansion. Indications include tension or traumatic pneumothorax after emergency decompression, significant hemothorax, empyema, and selected pleural effusions.

\medterm{Damage-Control Resuscitation}-- A trauma strategy prioritizing rapid hemorrhage control, permissive approaches to blood pressure when appropriate, prevention of hypothermia, correction of coagulopathy, and balanced blood-product resuscitation rather than large-volume crystalloid administration.

\medterm{Distributive Shock}-- Shock caused by pathologic vasodilation and maldistribution of blood flow, producing inadequate effective circulating volume despite variable cardiac output. Causes include sepsis, anaphylaxis, neurogenic shock, and some toxic syndromes.

\medterm{EFAST Examination}-- Extended focused assessment with sonography in trauma, a bedside ultrasound examination for intraperitoneal or pericardial free fluid and pneumothorax or hemothorax. It rapidly informs trauma resuscitation but does not exclude all internal injuries when negative.

\medterm{Intraosseous Access}-- Placement of a vascular-access needle into the medullary cavity of a suitable bone to permit rapid infusion of fluids, blood products, and medications when intravenous access is delayed or impossible. It is a temporary emergency route and can cause extravasation, infection, or compartment injury.

\medterm{Intraosseous Infusion}-- Administration of fluids, blood products, or medications through an intraosseous access device. Infusion should be monitored for resistance, swelling, pain, and extravasation and replaced with definitive vascular access as soon as practical.

\medterm{Massive Transfusion Protocol}-- A coordinated protocol for life-threatening hemorrhage that provides rapid, balanced delivery of red cells, plasma, and platelets with monitoring and correction of fibrinogen, calcium, temperature, and acid-base abnormalities.

\medterm{Noninvasive Ventilation}-- Positive-pressure ventilatory support delivered without an endotracheal tube, commonly by a mask or helmet. CPAP provides continuous positive pressure; bilevel support provides separate inspiratory and expiratory pressures. It is used selectively when the patient can protect the airway and cooperate.

\medterm{Point-of-Care Ultrasound}-- Bedside ultrasonography performed and interpreted by the treating clinician to answer focused diagnostic or procedural questions. Emergency applications include cardiac activity, volume status, pulmonary pathology, abdominal free fluid, vascular thrombosis, and procedural guidance.

\medterm{Procedural Sedation}-- Administration of sedative and analgesic medicines to facilitate a painful or anxiety-provoking procedure while preserving spontaneous ventilation and protective reflexes to the intended depth. Continuous monitoring and immediate rescue capability are required.

\medterm{Rapid-Sequence Intubation}-- A planned emergency airway technique using a sedative-hypnotic and a neuromuscular blocker in rapid succession to facilitate tracheal intubation while minimizing aspiration risk. Preparation includes preoxygenation, backup airway plans, hemodynamic assessment, and confirmation of tube position.

\medterm{Return of Spontaneous Circulation}-- Restoration of sustained cardiac mechanical activity after cardiac arrest, evidenced by a palpable pulse or measurable blood pressure and signs of circulation. Post-arrest care addresses oxygenation, ventilation, blood pressure, temperature, neurologic injury, and the cause of arrest.

\medterm{Septic Shock}-- A subset of sepsis with persistent circulatory and cellular dysfunction requiring vasopressors to maintain adequate blood pressure and associated with elevated mortality. Management requires prompt source control, appropriate antimicrobials, fluid assessment, and vasopressor support when indicated.

\medterm{Subarachnoid Hemorrhage}-- Bleeding into the subarachnoid space, most often from rupture of an intracranial aneurysm. It classically causes sudden maximal-intensity headache and may cause meningismus, vomiting, loss of consciousness, or focal neurologic deficits; urgent brain imaging and specialist management are required.

\medterm{Toxidrome}-- A recognizable pattern of signs and symptoms produced by a class of toxins. Important patterns include opioid, sympathomimetic, anticholinergic, cholinergic, sedative-hypnotic, and serotonin toxidromes; treatment combines supportive care, targeted antidotes when available, and decontamination decisions.

\medterm{Naloxone}-- An opioid-receptor antagonist used to reverse opioid-induced respiratory depression. It should be titrated to restore adequate ventilation rather than necessarily complete arousal, because abrupt reversal can precipitate acute withdrawal and severe pain.

\medterm{N-Acetylcysteine}-- An antidote that replenishes glutathione and enhances detoxification of the toxic metabolite of acetaminophen. It is most effective when started early but may benefit patients with delayed presentation or established liver injury under specialist guidance.

\medterm{Lumbar Puncture}-- Insertion of a needle into the lumbar subarachnoid space to measure cerebrospinal-fluid pressure or obtain fluid for analysis and, less commonly, administer treatment. It is contraindicated or delayed when there is concern for mass effect, obstructed CSF flow, or an uncorrected bleeding risk.

\medterm{Pelvic Binder}-- A circumferential device applied around the pelvis to reduce motion and hemorrhage from suspected unstable pelvic-ring injury. It should be centered over the greater trochanters and used with definitive imaging and hemorrhage-control management.

\medterm{Ectopic Pregnancy}-- Implantation of a pregnancy outside the endometrial cavity, most commonly in a fallopian tube. Rupture can cause life-threatening intra-abdominal hemorrhage; evaluation uses pregnancy testing, serial quantitative hCG, ultrasonography, and hemodynamic assessment.

\medterm{Postpartum Hemorrhage}-- Excessive blood loss after childbirth causing maternal compromise, commonly from uterine atony, retained placental tissue, genital-tract trauma, or coagulopathy. Management requires simultaneous resuscitation, identification of the cause, uterotonic or procedural treatment, and blood-product support when indicated.

\medterm{Neonatal Resuscitation}-- Immediate assessment and support of a newborn who does not establish effective breathing or circulation after birth. Interventions may include warmth, positioning, stimulation, ventilation, chest compressions, and medications according to neonatal resuscitation algorithms.

\medterm{Febrile Seizure}-- A seizure occurring in a child usually between 6 months and 5 years of age with fever but without evidence of central nervous system infection or a previous afebrile seizure. Simple febrile seizures are generalized, brief, and nonrecurrent within the same illness; prolonged or focal events require further evaluation.

\medterm{Acute Limb Ischemia}-- Sudden arterial hypoperfusion of a limb caused by embolism, thrombosis, graft occlusion, or trauma. Pain, pallor, pulselessness, paresthesia, paralysis, and poikilothermia are warning findings; threatened limbs require urgent vascular assessment and revascularization.

\medterm{Tourniquet}-- A device applied proximally to a limb to compress arterial inflow and control life-threatening extremity hemorrhage when direct pressure is inadequate or impractical. Application time, bleeding control, distal perfusion, and the need for definitive hemorrhage management must be documented.

\medterm{Airway Obstruction}-- Partial or complete blockage of airflow through the upper or lower respiratory tract. It may result from a foreign body, secretions, swelling, trauma, tumor, or loss of airway muscle tone and can cause stridor, wheeze, hypoxaemia, respiratory distress, or arrest. Severity is assessed clinically and requires prompt airway support when ventilation or oxygenation is threatened.

\medterm{Burn}-- See \hyperlink{Burns}{Burns}.

\medterm{Capillary Leak Syndrome}-- A disorder of increased capillary permeability in which plasma and proteins move from the intravascular space into surrounding tissues. It may cause hypotension, haemoconcentration, hypoalbuminaemia, oedema, and shock; causes include severe infection, immune-mediated disease, toxins, and rare idiopathic episodes.

\medterm{Chills}-- A subjective sensation of cold, often accompanied by shivering or piloerection, caused by hypothalamic thermoregulatory responses. Chills commonly occur during fever or infection but may also accompany exposure, endocrine or metabolic disturbance, medication effects, or acute systemic illness.

\medterm{Cold Injury}-- Tissue or systemic injury caused by exposure to low temperature, including hypothermia, frostbite, chilblains, and non-freezing cold injury. Severity depends on temperature, exposure duration, moisture, wind, and perfusion; affected patients require assessment for core-temperature disturbance and local tissue damage.

\medterm{Contusion}-- A closed soft-tissue injury caused by blunt force, producing bleeding into tissues without an open wound. It commonly causes pain, swelling, and ecchymosis; severe contusions may involve muscle necrosis, compartment syndrome, internal bleeding, or associated bone and organ injury.

\medterm{Gagging}-- A protective retching response triggered by stimulation of the pharynx, posterior tongue, or nearby structures. It may occur during swallowing, examination, aspiration, reflux, infection, or emotional distress; persistent gagging with dysphagia, vomiting, respiratory symptoms, or weight loss requires assessment.
