\medterm{Acute Liver Failure} Alternate name; see \textit{Acute Liver Failure}.
...
\medterm{Advance Directive} Alternate name; see \textit{Living Will}.
...
\medterm{Central Line} A catheter whose tip lies in a large central vein, used to deliver medicines or fluids, monitor central venous pressure, draw blood, or provide specialized nutrition.

\medterm{Central Venous Line} A central venous catheter inserted into a large vein such as the internal jugular, subclavian, or femoral vein and advanced toward the superior or inferior vena cava.
...
\medterm{False Labor} Alternate name; see \textit{Braxton Hicks Contractions}.
...
\medterm{Hepatic Encephalopathy} A spectrum of reversible neuropsychiatric abnormalities occurring in patients with acute liver failure or chronic advanced cirrhosis and portosystemic shunting, caused by accumulation of neurotoxic metabolites such as ammonia in the central nervous system.

\synonyms
Portosystemic Encephalopathy; Hepatic Coma; Liver Brain Dysfunction

\medterm{Hepatocellular Carcinoma} The most common primary malignancy of the liver, arising directly from hepatocytes. Strongly associated with cirrhosis secondary to chronic hepatitis B or C infection, alcohol-related liver disease, and metabolic dysfunction-associated steatohepatitis (MASH).

\synonyms
HCC; Primary Liver Cancer; Hepatoma

\medterm{Hodgkin Lymphoma} Alternate name; see \textit{Hodgkin Lymphoma}.
...
\medterm{Interstitial Lung Disease} Alternate name; see \textit{Interstitial Lung Disease}.
...
\medterm{Lacunar Infarct} A small (less than 15 mm in diameter) deep cerebral ischemic infarction caused by occlusion of a single penetrating branch artery arising from the circle of Willis, middle cerebral artery trunk, or basilar artery (such as the lenticulostriate or thalamoperforating arteries) due to lipohyalinosis or microatheroma.
\synonyms
...
\medterm{Laparoscopy} A minimally invasive surgical technique that enables surgeons to visualize and operate inside the abdomen and pelvis through tiny incisions rather than a large open cut. After creating a small incision near the navel, the abdominal cavity is gently inflated with harmless carbon dioxide gas to create working space and a clear visual field. A laparoscope---a slender, lighted tube fitted with a high-definition digital camera---is inserted to transmit magnified live images to a video screen. Working through two to three additional tiny puncture cuts (keyhole incisions), delicate specialized instruments can perform diagnostic evaluations (such as investigating chronic pelvic pain, endometriosis, ectopic pregnancy, or unexplained infertility) or major therapeutic surgeries (such as removing the appendix, gallbladder, kidney, or uterus). Compared to traditional open surgery (laparotomy), laparoscopy results in significantly smaller scars, less postoperative pain and blood loss, a reduced risk of wound infection, shorter hospital stays, and a much faster return to normal daily life.
...
\medterm{Lateral Medullary Syndrome} A classic brainstem stroke syndrome caused by infarction of the posterolateral medulla oblongata, typically resulting from occlusion of the posterior inferior cerebellar artery (PICA) or vertebral artery, presenting with ipsilateral facial pain and temperature loss, Horner syndrome, cerebellar ataxia, dysphagia, dysarthria, and contralateral trunk/extremity sensory loss.
\synonyms
...
\medterm{Lead Pipe Rigidity} A sustained, uniform, plastic resistance to passive movement that persists throughout the entire range of motion of a joint without fluctuation or cogwheeling, characteristic of severe extrapyramidal disease, advanced Parkinson disease, and neuroleptic malignant syndrome.
\synonyms
...
\medterm{Left Ventricular Assist Device} A surgically implanted mechanical circulatory support pump used to assist a failing left ventricle in patients with advanced end-stage heart failure. During open-heart surgery, the pump is connected directly to the apex of the left ventricle and pumps oxygen-rich blood continuously through an outflow conduit into the ascending aorta, effectively taking over the primary pumping work of the heart. A small driveline cable exits the abdominal skin to connect the internal pump to a wearable external computer controller and battery pack. An LVAD is indicated as a ``bridge to transplant'' (sustaining life while the patient awaits a compatible donor heart) or as ``destination therapy'' (permanent long-term therapy for individuals who are not candidates for heart transplantation), significantly improving survival, physical stamina, and quality of life.
...
\medterm{Leigh Syndrome} An inherited, progressive neurometabolic disorder (subacute necrotizing encephalomyelopathy) caused by genetic defects in mitochondrial respiratory chain complexes or pyruvate dehydrogenase, characterized pathologically by bilateral symmetrical necrotic spongiform lesions within the basal ganglia, thalamus, and brainstem.
\synonyms
...
\medterm{Leukemia, Acute Lymphocytic} Alternate name; see \textit{Acute Lymphocytic Leukemia}.

\medterm{Leukemia, Acute Myelogenous} Alternate name; see \textit{Acute Myelogenous Leukemia}.

\medterm{Leukemia, Chronic Lymphocytic} Alternate name; see \textit{Chronic Lymphocytic Leukemia}.

\medterm{Leukemia, Chronic Myelogenous} Alternate name; see \textit{Chronic Myelogenous Leukemia}.
...
\medterm{Leukemia, General} Alternate name; see \textit{Leukemia}.
...
\medterm{Leukemia, Hairy Cell} Alternate name; see \textit{Hairy Cell Leukemia}.
...
\medterm{Lhermitte-Duclos Disease} A rare, benign, hamartomatous overgrowth of the cerebellar cortex (dysplastic gangliocytoma of the cerebellum) characterized by widening of cerebellar folia, marked hypertrophy of the granule cell layer, and loss of Purkinje cells, serving as a pathognomonic diagnostic criterion for Cowden syndrome (PTEN hamartoma tumor syndrome).
\synonyms
...
\medterm{Lice, Body} Alternate name; see \textit{Body Lice}.
...
\medterm{Lichen Planus, Oral} Alternate name; see \textit{Oral Lichen Planus}.
...
\medterm{Lipitor} Lipitor is a brand name for atorvastatin, a statin medicine that lowers low-density lipoprotein cholesterol and reduces the risk of heart attack and stroke. Muscle symptoms and liver-related effects should be discussed with a clinician.
...
\medterm{Lower Gastrointestinal Bleeding} Bleeding that originates in the lower digestive tract, including the small intestine, colon, rectum, or anus. It most often appears as bright red or dark maroon blood passed with bowel movements, or as hidden microscopic blood leading to anemia; common causes include diverticular bleeding, hemorrhoids, polyps, colon cancer, and colitis. Heavy bleeding, dizziness, or fainting requires immediate emergency medical care.
...
\medterm{Lupus Band Test} A direct immunofluorescence (DIF) diagnostic test performed on a skin biopsy specimen in which continuous granular or linear deposition of immunoglobulins (IgG, IgM, IgA) and complement components (C3) is identified along the dermoepidermal junction (basement membrane zone), characteristic of systemic lupus erythematosus.
\synonyms
...
\medterm{Lymph Nodes, Swollen} Alternate name; see \textit{Swollen Lymph Nodes}.
...
\medterm{Mesenteric Lymphadenitis} Alternate name; see \textit{Mesenteric Lymphadenitis}.

\medterm{Neonatal Hypocalcemia} Abnormally low serum total calcium ($<8.0\text{ mg/dL}$ in term infants or $<7.0\text{ mg/dL}$ in preterm infants) or ionized calcium ($<4.0\text{ mg/dL}$) during infancy. Presents with neuromuscular irritability, jitteriness, tetany, stridor, or seizures.

\synonyms
Infant Hypocalcemia; Pediatric Low Calcium; Neonatal Tetany

\medterm{Neonatal Hypoglycemia} An abnormally low plasma glucose level in a newborn infant (typically defined as blood glucose $<40\text{ mg/dL}$ in the first 4 hours of life or $<45\text{ mg/dL}$ thereafter). Risk factors include maternal diabetes, intrauterine growth restriction, prematurity, and perinatal asphyxia.

\synonyms
Newborn Hypoglycemia; Infantile Low Blood Sugar; Neonatal Low Blood Glucose

\medterm{Non-Hodgkin Lymphoma} Alternate name; see \textit{Non-Hodgkin Lymphoma}.
...
\medterm{Small Cell Lung Cancer} An aggressive, highly malignant neuroendocrine carcinoma of the bronchopulmonary system strongly associated with cigarette smoking. Characterized by rapid doubling time, early widespread hematogenous metastases, and frequent paraneoplastic syndromes.

\synonyms
SCLC; Small Cell Bronchogenic Carcinoma; Oat Cell Carcinoma

\medterm{Solar Lentigines} Alternate name; see \textit{Age Spots}.

\medterm{Swollen Lymph Glands} Alternate name; see \textit{Swollen Lymph Nodes}.
...
\medterm{Vascular Laser Therapy} A laser system used to treat selected blood-vessel problems or tissue blocking a vessel. The energy is delivered through the skin or a catheter and must be carefully targeted to avoid damage to nearby tissue.
\synonyms
