...
\medterm{Idiosyncratic Reaction} An unusual and unexpected reaction to a medicine that is not explained by its usual effects and may occur even at a normal dose.
...
\medterm{Ileostomy} An operation that brings the end of the small intestine through an opening in the abdomen so stool can drain into an external pouch. The output is often liquid, so fluids and skin care are important.
...
count <100,000/L), usually with otherwise preserved white blood cell and hemoglobin levels. Caused by
...
\medterm{Impotence (Erectile Dysfunction [ED])} The persistent inability to attain or maintain a penile erection of sufficient rigidity for satisfactory sexual intercourse. Pathophysiologically categorized into vasculogenic (arteriogenic insufficiency from atherosclerosis, veno-occlusive dysfunction), neurogenic (diabetic autonomic neuropathy, radical prostatectomy, spinal cord injury), endocrinologic (hypogonadism, hyperprolactinemia), drug-induced (antihypertensives, SSRIs, antiandrogens), and psychogenic etiologies. Often serves as an early harbinger of occult systemic coronary artery disease due to endothelial dysfunction in smaller penile arteries; evaluated with fasting glucose/HbA1c, lipid panels, and morning serum testosterone, and treated first-line with oral phosphodiesterase-5 (PDE5) inhibitors (e.g., sildenafil, tadalafil).
...
\medterm{In Situ} Confined to the exact site of anatomical origin without invasion into adjacent tissue planes or across anatomical barriers. In surgical pathology and oncology, carcinoma in situ (CIS; e.g., ductal carcinoma in situ [DCIS] of the breast, high-grade squamous intraepithelial lesion [HSIL] of the cervix) denotes a malignant epithelial proliferation that has not breached the basement membrane (basal lamina); because lymphovascular structures reside exclusively in the underlying stroma, lesions strictly in situ possess zero potential for regional lymphatic or distant hematogenous metastasis.
...
\medterm{Indwelling Pleural Catheter} A small-bore, flexible silicone tube with a polyester cuff and a one-way valve mechanism (such as the PleurX catheter) placed percutaneously into the pleural space and tunneled subcutaneously through the chest wall. It allows safe, intermittent outpatient drainage of recurrent symptomatic pleural effusions (most commonly malignant pleural effusion or trapped lung) by the patient or a caregiver, relieving dyspnea without requiring repeated thoracentesis or prolonged hospital stays, and often inducing spontaneous pleurodesis.
...
\medterm{Inferior} In standard anatomical directional terminology, situated below, downward, or directed toward the feet or caudal extremity of the body relative to another anatomical landmark (the opposite of superior). For example, the diaphragm lies inferior to the thoracic lungs and heart, and the inferior vena cava (IVC) drains venous blood from structures located inferior to the anatomical respiratory diaphragm.
...
\medterm{Infundibulopelvic Ligament} A prominent peritoneal fold extending from the tubal pole of each ovary to the lateral pelvic sidewall. Serves as the vital anatomical conduit transmitting the ovarian artery, ovarian venous plexus, lymphatic channels, and autonomic nerve fibers to and from the ovary; represents the critical vascular pedicle isolated, doubly ligated, and divided during oophorectomy, with surgical care required to avoid injuring the underlying ureter crossing into the pelvis.
...
\medterm{Inspiration (Inhalation)} The active physiological phase of the respiratory ventilatory cycle during which ambient air is drawn into the lungs through the tracheobronchial tree. Initiated by contraction of the diaphragm (innervated by phrenic nerves C3--C5), which flattens downward, along with external intercostal muscle contraction, expanding thoracic cavity dimensions. This generates negative intrapleural and intrapulmonary (subatmospheric) pressure relative to atmospheric pressure, overcoming airway resistance and lung elastic recoil to inflate the pulmonary alveoli until alveolar pressure equilibrates with atmospheric pressure.
...
\medterm{Instantaneous Wave-Free Ratio} A resting invasive physiological index of coronary artery stenosis severity measured during catheterization using a specialized intracoronary pressure wire. Unlike fractional flow reserve (FFR), which requires maximal pharmacological hyperemia induced by adenosine, iFR calculates the ratio of distal coronary pressure to aortic pressure ($P_d/P_a$) specifically during a designated wave-free period in diastole when intracoronary microvascular resistance is naturally minimal and constant. Large randomized trials (DEFINE-FLAIR and iFR-SWEDEHEART) confirmed that an iFR threshold of $\le 0.89$ is non-inferior to FFR ($\le 0.80$) for guiding revascularization.
...
\medterm{INTERMACS Profile} A standardized, internationally recognized 7-tier clinical staging classification established by the Interagency Registry for Mechanically Assisted Circulatory Support (INTERMACS) to stratify the severity of illness and hemodynamic compromise in patients with advanced, end-stage (Stage D) heart failure. Classifies patients from Profile 1 (``crash and burn'': critical cardiogenic shock with persistent severe hypoperfusion despite escalating inotropic support) through Profile 2 (progressive decline on inotropes), Profile 3 (inotrope-dependent stability), Profile 4 (recurrent frequent hospitalizations), Profile 5 (housebound/exertion intolerant), Profile 6 (exertion limited), to Profile 7 (advanced NYHA Class III), guiding clinical urgency, patient selection, and timing for mechanical circulatory support (VAD) implantation or emergency heart transplantation.
...
\medterm{International Unit (IU)} A standardized internationally accepted unit of measurement for quantifying the biological activity, potency, or pharmacological effect of a substance (such as fat-soluble vitamins, insulin, heparin, and vaccines), established by the World Health Organization (WHO). Unlike metric mass units (milligrams or micrograms), the exact mass equivalent of one IU varies depending on the specific biological substance and reference preparation.
...
\medterm{Intracavernosal Injection} A second-line treatment for erectile dysfunction involving the direct self-injection of vasoactive medications into the corpus cavernosum of the penis using a fine-gauge needle. The injected agents (commonly alprostadil [prostaglandin E1] as monotherapy, or combinations with papaverine and phentolamine known as Bimix or Trimix) stimulate smooth-muscle relaxation and arterial dilation within the cavernosal sinusoids to induce an erection independent of sexual stimulation. It is effective in organic, vasculogenic, and neurogenic erectile dysfunction (such as following radical prostatectomy); potential complications include localized pain, hematoma, cavernosal fibrosis, and prolonged erection or ischemic priapism requiring urgent intervention.
...
\medterm{Intracellular} Pertaining to, positioned inside, or occurring within the bounding plasma membrane of a biological cell. The intracellular fluid (ICF) compartment comprises approximately two-thirds of total body water in adults, characterized electrochemically by high concentrations of potassium ($\text{K}^+$), magnesium ($\text{Mg}^{2+}$), and organic phosphate/protein polyanions, and low concentrations of sodium ($\text{Na}^+$) and chloride ($\text{Cl}^-$), maintained continuously across the plasma membrane by ATP-dependent $\text{Na}^+/\text{K}^+$-ATPase pump activity.
...
\medterm{Intracranial} Situated or occurring within the bony cranium (skull cavity). The rigid, non-distensible cranial vault contains three physiological volume components: brain parenchyma ($\approx 80\%$), cerebrospinal fluid (CSF; $\approx 10\%$), and intracranial blood ($\approx 10\%$). Governed by the Monro-Kellie doctrine, an increase in any one intracranial volume component (e.g., from cerebral edema, intracranial hemorrhage, or brain tumor) must be compensated by a reciprocal reduction in CSF or venous blood; when compensatory capacity is exhausted, intracranial pressure (ICP) rises steeply ($>20~\text{mmHg}$), risking cerebral hypoperfusion, ischemic brain injury, and fatal brain herniation syndromes.
...
\medterm{Intrahepatic} Situated, occurring, or originating within the anatomical parenchyma or capsule of the liver. Clinically relevant in hepatobiliary disease, including intrahepatic cholestasis (impaired bile formation or secretion by hepatocytes or small intrahepatic cholangioles, as in primary biliary cholangitis), intrahepatic bile duct dilatation on ultrasound (differentiating intrahepatic biliary obstruction from extrahepatic choledocholithiasis), and primary intrahepatic malignancies (hepatocellular carcinoma and intrahepatic cholangiocarcinoma).
...
\medterm{Intramuscular (IM)} Pertaining to the interior of a skeletal muscle, or describing a parenteral route of medication administration injected deep into muscular tissue (commonly the deltoid, vastus lateralis, or ventrogluteal muscles). Provides more rapid systemic vascular absorption than subcutaneous injection owing to greater muscle vascularity, suitable for vaccines, depot antipsychotics, and medications unsuited for oral or IV administration.
...
\medterm{Intraocular} Situated, occurring, or administered within the eyeball (globe). The intraocular compartment is divided into the anterior segment (anterior and posterior chambers filled with clear aqueous humor) and the posterior segment (vitreous chamber filled with gelatinous vitreous humor). Maintained at a normal intraocular pressure (IOP; $10{-}21~\text{mmHg}$) by a delicate balance between aqueous humor secretion by the ciliary body epithelium and outflow through the trabecular meshwork; chronically elevated intraocular pressure is the primary modifiable risk factor for glaucomatous optic neuropathy and irreversible visual field loss.
...
\medterm{Ipsilateral} Pertaining to, situated on, or affecting the same side of the body relative to a given point of reference or pathological lesion (the opposite of contralateral). In clinical neurology, unilateral cerebellar hemisphere lesions produce ipsilateral limb ataxia, dysmetria, and intention tremor, and a disruption of the cervical sympathetic chain causes ipsilateral Horner syndrome (ptosis, miosis, and anhidrosis on the same side as the lesion).
...
\medterm{Isovaleric Acidemia} An autosomal recessive inborn error of leucine catabolism caused by a deficiency of the mitochondrial enzyme isovaleryl-CoA dehydrogenase (\textit{IVD}). Results in the pathological accumulation of isovaleryl-CoA and its derivatives, notably isovaleric acid, isovalerylglycine, and isovalerylcarnitine. Characterized by intermittent acute episodes of severe metabolic ketoacidosis, vomiting, lethargy, encephalopathy, cerebral edema, and a distinctive, pungent odor resembling ``sweaty feet'' or stale cheese, accompanied by hyperammonemia, pancytopenia, and hypocalcemia during acute decompensation.
...
\medterm{ISUP Grade Group} The standardized five-tier pathological grading system established by the International Society of Urological Pathology (ISUP) and endorsed by the World Health Organization (WHO) to stage prostate adenocarcinoma, addressing the clinical ambiguity of historical Gleason scoring. The system stratifies cancer prognosis into Grade Group 1 (Gleason score $\le 6$, indolent with excellent prognosis), Grade Group 2 (Gleason score $3+4=7$, predominantly favorable prognosis), Grade Group 3 (Gleason score $4+3=7$, unfavorable prognosis with predominant pattern 4), Grade Group 4 (Gleason score 8, high risk), and Grade Group 5 (Gleason score 9--10, highest risk of systemic progression and mortality). It provides patients and clinicians with clearer risk stratification for active surveillance versus definitive local or systemic therapy.
