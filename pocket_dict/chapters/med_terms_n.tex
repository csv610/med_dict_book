...
\medterm{Needle Decompression} A critical, life-saving emergency procedure performed immediately when a patient develops a tension pneumothorax---a condition where air builds up under positive pressure within the pleural space, collapsing the lung and compressing the heart and great veins, causing rapid shock and fatal arrest. A clinician inserts a large-bore needle with a catheter (typically 14-gauge or 16-gauge, at least 5 cm long) directly through the chest wall into the pleural cavity, commonly placed in the second intercostal space at the midclavicular line or the fourth or fifth intercostal space along the anterior axillary line. The sudden release of trapped air under pressure produces an audible hiss, instantly relieving intrathoracic tension, restoring venous blood return to the heart, and normalizing blood pressure. Needle decompression converts a life-threatening tension pneumothorax into a simple open pneumothorax, serving as an emergency temporizing bridge until a definitive tube thoracostomy (chest tube) can be inserted.
...
\medterm{Needle Holder} Alternate index label for Needle Driver.
...
\medterm{Neurocognitive Disorders} Alternate index label for Neurocognitive Disorder.
...
\medterm{Non-Invasive Procedure} A medical examination, diagnostic test, or treatment that does not cut or pierce the skin, puncture blood vessels, or enter internal body cavities or organs. Because the body's natural barriers remain intact, non-invasive procedures carry minimal physical risk, cause little or no discomfort, avoid the need for anesthesia or incisions, and require no surgical recovery time. Common examples include diagnostic imaging (such as standard X-rays, ultrasound, MRI, and CT scans), physiological monitoring (such as measuring blood pressure, taking an electrocardiogram [ECG], checking pulse oximetry, and hearing tests), and external therapies (such as non-invasive mask ventilation, physical therapy, and external bone splinting). They are usually the first step in medical evaluation before more invasive interventions are considered.
...
\medterm{Nasal Bleeding} Alternate index label for Nosebleeds.
